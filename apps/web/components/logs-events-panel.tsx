...
export function LogsEventsPanel({ programId }: { programId: string }) {
  ...
  const [saved, setSaved] = useState<Array<{ id: string; name: string; queryJson: Record<string, unknown> }>>([]);
  const [live, setLive] = useState(false);

  useEffect(() => {
    getSavedSearches(programId).then((r) => {
      setSaved((r.rows as Array<{ id: string; name: string; queryJson: Record<string, unknown> }>) ?? []);
    });
  }, [programId]);

  const runSearch = async (query = q) => {
    const res = await searchProgramLogs(programId, { q: query, filters: {}, limit: 100 });
  ...
  useEffect(() => {
    ...
    ws.onmessage = (evt) => {
      ...
      const line = Array.isArray(payload.log_lines) ? String(payload.log_lines[0] ?? "") : "";
      setRows((prev) => [
        {
    ...
    <div className="grid gap-4 lg:grid-cols-[1fr_280px]">
      <div className="space-y-3 rounded-md border p-4">
        <div className="flex gap-2">
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search logs and events" />
        ...
        <div className="space-y-2">
          {highlighted.map((r) => (
            <div key={`${r.signature}-${r.slot}`} className="rounded border p-2 text-xs">
              <div className="flex items-center justify-between">
                <span>{new Date(r.block_time).toLocaleString()}</span>
                <span className={r.status === "failed" ? "text-red-600" : "text-emerald-600"}>{r.status}</span>
              </div>
              <div className="font-mono">{r.signature}</div>
              <div>{r.instruction ?? "unknown"} · {r.signer}</div>
      ...
      <div className="space-y-3 rounded-md border p-4">
        ...
        <Button
          ...
          onClick={async () => {
            ...
            setSaved((next.rows as Array<{ id: string; name: string; queryJson: Record<string, unknown> }>) ?? []);
          }}
        >
          Save current search
        </Button>
        <div className="space-y-2">
          {saved.map((s) => (
            <button
              ...
              onClick={() => {
                const qq = typeof s.queryJson?.q === "string" ? s.queryJson.q : "";
                setQ(qq);
                runSearch(qq);
              }}
            >
