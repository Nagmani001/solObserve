...
export function AlertsPanel({ programId }: { programId: string }) {
  ...
  async function reload() {
    const [r, i] = await Promise.all([
      getAlertRules(programId),
      getAlertIncidents(programId),
    ]);
        ...
        <div className="space-y-2">
          {incidents.map((inc) => (
            <button
              ...
              onClick={async () =>
                setSelected(await getAlertIncident(programId, inc.id))
              }
            >
              <div className="font-medium">
                {inc.rule?.name ?? "rule"} · {inc.summary}
              </div>
              <div className="text-muted-foreground">
          ...
          <div className="space-y-2 rounded border p-3 text-xs">
            ...
            <div className="flex gap-2">
              <Button
                ...
                variant="outline"
                onClick={async () => {
                  const id = ((selected.incident as { id?: string })?.id ??
                    "") as string;
                  if (!id) return;
              ...
              <Button
                ...
                onClick={async () => {
                  const id = ((selected.incident as { id?: string })?.id ??
                    "") as string;
                  if (!id) return;
                  await alertIncidentAction(programId, id, {
                    action: "resolve",
                  });
                  await reload();
                }}
